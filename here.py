...
# Initialize sensors
touch_sensor = sensors.TouchSensor('in1')
color_sensor = sensors.ColorSensor('in3')

# Initialize motors
left_motor = motors.LargeMotor('outB')
right_motor = motors.LargeMotor('outD')
...
def move_forward_until_dark_surface():
    left_motor.on(FORWARD_SPEED)
    right_motor.on(FORWARD_SPEED)
    while color_sensor.reflected_light_intensity > 30:  # Adjust the threshold as needed
        time.sleep(0.1)
    left_motor.off()
    right_motor.off()
